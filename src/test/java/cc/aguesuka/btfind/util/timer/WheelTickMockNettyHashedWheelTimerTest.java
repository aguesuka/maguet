 ...
 * 2020/4/8 21:08
 */
@SuppressWarnings({"Convert2Lambda", "RedundantThrows", "Convert2Diamond"})
public class WheelTickMockNettyHashedWheelTimerTest {
    ...
    private static class HashedWheelTimer implements Timer {
        ...
        private HashedWheelTimer() {
            this(100, TimeUnit.MILLISECONDS, 10000, System::currentTimeMillis);
        }
        ...
        public synchronized Timeout newTimeout(TimerTask task, long delay, TimeUnit unit) {
            if (maxPendingTimeouts != 0 && maxPendingTimeouts <= this.pendingTimeouts()) {
            ...
            synchronized (tick) {
                var timeout = new Timeout() {
                    cc.aguesuka.btfind.util.timer.Timeout timeout;
                    ...
                    public boolean cancel() {
                        canceled = true;
                        timeoutSet.remove(this);
                        return timeout.cancel();
                    }
                };

                long mill = tick.timeUnit.convert(delay, unit);

                if (mill > 0 && tick.wheelDuration - mill < tick.tickDuration) {
                    delay = tick.wheelDuration - tick.tickDuration;
                    unit = tick.timeUnit;
                }
                timeout.timeout = tick.add(() -> {
